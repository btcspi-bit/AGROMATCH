...
export function DemandCard({
  ...
  const safeTitle = title || "Compra sem título";
  const safeOwner = owner || "Comprador";
  ...
  async function handleShare(event: any) {
    ...
    const shareText = [
      ...
      price ? `Valor: ${price}` : "",
      "",
      "Confira essa procura no AgroMatch.",
    ]
      .filter(Boolean)
      .join("\n");
    ...
    try {
      ...
      if (typeof navigator !== "undefined" && navigator.clipboard) {
        ...
        alert("Procura copiada para compartilhar.");
      }
    } catch (error) {
      console.error("Erro ao compartilhar procura:", error);
    }
  }
    ...
    >
      ...
      <div style={statusLine}>
        <span style={isSold ? soldBadge : isPaused ? pausedBadge : activeBadge}>
          {isSold ? "Atendida" : isPaused ? "Pausada" : "Comprador ativo"}
        </span>
      ...
      <p style={lineStrong}>
        Procura: {breed || "Raça aberta"} • {gender || "Sexo aberto"} •{" "}
      ...
      <div style={chipsGrid}>
        ...
        <InfoChip label="Negociação" value={negotiationText} highlight />
      </div>
      ...
      <div style={footerLine}>
        <span style={roadStyle}>
          {isSold
            ? "Essa compra já foi atendida"
            : road || distance || "Raio não informado"}
        </span>
